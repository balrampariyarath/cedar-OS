...
const cedarUiPackagePath = path.join(__dirname, '../packages/cedar/package.json');
...
if (mainPackage.dependencies['cedar'] === 'workspace:*') {
  mainPackage.dependencies['cedar'] = `^${cedarUiVersion}`;
  changed = true;
  console.log(`✅ Replaced cedar workspace:* with ^${cedarUiVersion} for production build`);
} else {
  console.log(`ℹ️  cedar dependency is already set to: ${mainPackage.dependencies['cedar']}`);
}
